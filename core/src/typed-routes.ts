...
> = (context: {
  ...
  body: z.infer<TBody>;
  req: Request<z.infer<TParams>, z.infer<TBody>, z.infer<TQuery>>;
...
> {
  ...
  handler(handler: TypedHandle<RouteQuery, RouteParams, RouteBody>): HandlerMetadata {
    const invokeHandler = async (req: Request, res: Response) => {
      let message = '', query, params, body
      try {
        message = 'Query';
